<template>
  <div>
    <svg
      style="position: absolute; width: 0; height: 0; overflow: hidden;"
      version="1.1"
      xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink"
    >
      <defs>
        <symbol id="icon-quill" viewBox="0 0 32 32">
          <title>quill</title>
          <path
            d="M0 32c4-12 14.469-32 32-32-8.219 6.594-12 22-18 22s-6 0-6 0l-6 10h-2z"
          />
        </symbol>
        <symbol id="icon-library" viewBox="0 0 34 32">
          <title>library</title>
          <path
            d="M32 30v-2h-2v-12h2v-2h-6v2h2v12h-6v-12h2v-2h-6v2h2v12h-6v-12h2v-2h-6v2h2v12h-6v-12h2v-2h-6v2h2v12h-2v2h-2v2h34v-2h-2z"
          />
          <path d="M16 0h2l16 10v2h-34v-2l16-10z" />
        </symbol>
        <symbol id="icon-profile" viewBox="0 0 32 32">
          <title>profile</title>
          <path
            d="M27 0h-24c-1.65 0-3 1.35-3 3v26c0 1.65 1.35 3 3 3h24c1.65 0 3-1.35 3-3v-26c0-1.65-1.35-3-3-3zM26 28h-22v-24h22v24zM8 18h14v2h-14zM8 22h14v2h-14zM10 9c0-1.657 1.343-3 3-3s3 1.343 3 3c0 1.657-1.343 3-3 3s-3-1.343-3-3zM15 12h-4c-1.65 0-3 0.9-3 2v2h10v-2c0-1.1-1.35-2-3-2z"
          />
        </symbol>
        <symbol id="icon-phone" viewBox="0 0 32 32">
          <title>phone</title>
          <path
            d="M22 20c-2 2-2 4-4 4s-4-2-6-4-4-4-4-6 2-2 4-4-4-8-6-8-6 6-6 6c0 4 4.109 12.109 8 16s12 8 16 8c0 0 6-4 6-6s-6-8-8-6z"
          />
        </symbol>
        <symbol id="icon-location2" viewBox="0 0 32 32">
          <title>location2</title>
          <path
            d="M16 0c-5.523 0-10 4.477-10 10 0 10 10 22 10 22s10-12 10-22c0-5.523-4.477-10-10-10zM16 16.125c-3.383 0-6.125-2.742-6.125-6.125s2.742-6.125 6.125-6.125 6.125 2.742 6.125 6.125-2.742 6.125-6.125 6.125zM12.125 10c0-2.14 1.735-3.875 3.875-3.875s3.875 1.735 3.875 3.875c0 2.14-1.735 3.875-3.875 3.875s-3.875-1.735-3.875-3.875z"
          />
        </symbol>
        <symbol id="icon-display" viewBox="0 0 32 32">
          <title>display</title>
          <path
            d="M0 2v20h32v-20h-32zM30 20h-28v-16h28v16zM21 24h-10l-1 4-2 2h16l-2-2z"
          />
        </symbol>
        <symbol id="icon-mobile" viewBox="0 0 32 32">
          <title>mobile</title>
          <path
            d="M23 0h-14c-1.65 0-3 1.35-3 3v26c0 1.65 1.35 3 3 3h14c1.65 0 3-1.35 3-3v-26c0-1.65-1.35-3-3-3zM12 1.5h8v1h-8v-1zM16 30c-1.105 0-2-0.895-2-2s0.895-2 2-2 2 0.895 2 2-0.895 2-2 2zM24 24h-16v-20h16v20z"
          />
        </symbol>
        <symbol id="icon-tablet" viewBox="0 0 32 32">
          <title>tablet</title>
          <path
            d="M25 0h-20c-1.65 0-3 1.35-3 3v26c0 1.65 1.35 3 3 3h20c1.65 0 3-1.35 3-3v-26c0-1.65-1.35-3-3-3zM15 31c-0.552 0-1-0.448-1-1s0.448-1 1-1 1 0.448 1 1-0.448 1-1 1zM24 28h-18v-24h18v24z"
          />
        </symbol>
        <symbol id="icon-database" viewBox="0 0 32 32">
          <title>database</title>
          <path
            d="M16 0c-8.837 0-16 2.239-16 5v4c0 2.761 7.163 5 16 5s16-2.239 16-5v-4c0-2.761-7.163-5-16-5z"
          />
          <path
            d="M16 17c-8.837 0-16-2.239-16-5v6c0 2.761 7.163 5 16 5s16-2.239 16-5v-6c0 2.761-7.163 5-16 5z"
          />
          <path
            d="M16 26c-8.837 0-16-2.239-16-5v6c0 2.761 7.163 5 16 5s16-2.239 16-5v-6c0 2.761-7.163 5-16 5z"
          />
        </symbol>
        <symbol id="icon-bubbles2" viewBox="0 0 36 32">
          <title>bubbles2</title>
          <path
            d="M15 0v0c8.284 0 15 5.435 15 12.139s-6.716 12.139-15 12.139c-0.796 0-1.576-0.051-2.339-0.147-3.222 3.209-6.943 3.785-10.661 3.869v-0.785c2.008-0.98 3.625-2.765 3.625-4.804 0-0.285-0.022-0.564-0.063-0.837-3.392-2.225-5.562-5.625-5.562-9.434 0-6.704 6.716-12.139 15-12.139zM31.125 27.209c0 1.748 1.135 3.278 2.875 4.118v0.673c-3.223-0.072-6.181-0.566-8.973-3.316-0.661 0.083-1.337 0.126-2.027 0.126-2.983 0-5.732-0.805-7.925-2.157 4.521-0.016 8.789-1.464 12.026-4.084 1.631-1.32 2.919-2.87 3.825-4.605 0.961-1.84 1.449-3.799 1.449-5.825 0-0.326-0.014-0.651-0.039-0.974 2.268 1.873 3.664 4.426 3.664 7.24 0 3.265-1.88 6.179-4.82 8.086-0.036 0.234-0.055 0.474-0.055 0.718z"
          />
        </symbol>
        <symbol id="icon-quotes-left" viewBox="0 0 32 32">
          <title>quotes-left</title>
          <path
            d="M7.031 14c3.866 0 7 3.134 7 7s-3.134 7-7 7-7-3.134-7-7l-0.031-1c0-7.732 6.268-14 14-14v4c-2.671 0-5.182 1.040-7.071 2.929-0.364 0.364-0.695 0.751-0.995 1.157 0.357-0.056 0.724-0.086 1.097-0.086zM25.031 14c3.866 0 7 3.134 7 7s-3.134 7-7 7-7-3.134-7-7l-0.031-1c0-7.732 6.268-14 14-14v4c-2.671 0-5.182 1.040-7.071 2.929-0.364 0.364-0.695 0.751-0.995 1.157 0.358-0.056 0.724-0.086 1.097-0.086z"
          />
        </symbol>
        <symbol id="icon-quotes-right" viewBox="0 0 32 32">
          <title>quotes-right</title>
          <path
            d="M25 20c-3.866 0-7-3.134-7-7s3.134-7 7-7 7 3.134 7 7l0.031 1c0 7.732-6.268 14-14 14v-4c2.671 0 5.182-1.040 7.071-2.929 0.364-0.364 0.695-0.75 0.995-1.157-0.357 0.056-0.724 0.086-1.097 0.086zM7 20c-3.866 0-7-3.134-7-7s3.134-7 7-7 7 3.134 7 7l0.031 1c0 7.732-6.268 14-14 14v-4c2.671 0 5.182-1.040 7.071-2.929 0.364-0.364 0.695-0.75 0.995-1.157-0.357 0.056-0.724 0.086-1.097 0.086z"
          />
        </symbol>
        <symbol id="icon-share2" viewBox="0 0 32 32">
          <title>share2</title>
          <path
            d="M27 22c-1.411 0-2.685 0.586-3.594 1.526l-13.469-6.734c0.041-0.258 0.063-0.522 0.063-0.791s-0.022-0.534-0.063-0.791l13.469-6.734c0.909 0.94 2.183 1.526 3.594 1.526 2.761 0 5-2.239 5-5s-2.239-5-5-5-5 2.239-5 5c0 0.269 0.022 0.534 0.063 0.791l-13.469 6.734c-0.909-0.94-2.183-1.526-3.594-1.526-2.761 0-5 2.239-5 5s2.239 5 5 5c1.411 0 2.685-0.586 3.594-1.526l13.469 6.734c-0.041 0.258-0.063 0.522-0.063 0.791 0 2.761 2.239 5 5 5s5-2.239 5-5c0-2.761-2.239-5-5-5z"
          />
        </symbol>
        <symbol id="icon-twitter" viewBox="0 0 32 32">
          <title>twitter</title>
          <path
            d="M32 7.075c-1.175 0.525-2.444 0.875-3.769 1.031 1.356-0.813 2.394-2.1 2.887-3.631-1.269 0.75-2.675 1.3-4.169 1.594-1.2-1.275-2.906-2.069-4.794-2.069-3.625 0-6.563 2.938-6.563 6.563 0 0.512 0.056 1.012 0.169 1.494-5.456-0.275-10.294-2.888-13.531-6.862-0.563 0.969-0.887 2.1-0.887 3.3 0 2.275 1.156 4.287 2.919 5.463-1.075-0.031-2.087-0.331-2.975-0.819 0 0.025 0 0.056 0 0.081 0 3.181 2.263 5.838 5.269 6.437-0.55 0.15-1.131 0.231-1.731 0.231-0.425 0-0.831-0.044-1.237-0.119 0.838 2.606 3.263 4.506 6.131 4.563-2.25 1.762-5.075 2.813-8.156 2.813-0.531 0-1.050-0.031-1.569-0.094 2.913 1.869 6.362 2.95 10.069 2.95 12.075 0 18.681-10.006 18.681-18.681 0-0.287-0.006-0.569-0.019-0.85 1.281-0.919 2.394-2.075 3.275-3.394z"
          />
        </symbol>
        <symbol id="icon-github" viewBox="0 0 32 32">
          <title>github</title>
          <path
            d="M16 0.395c-8.836 0-16 7.163-16 16 0 7.069 4.585 13.067 10.942 15.182 0.8 0.148 1.094-0.347 1.094-0.77 0-0.381-0.015-1.642-0.022-2.979-4.452 0.968-5.391-1.888-5.391-1.888-0.728-1.849-1.776-2.341-1.776-2.341-1.452-0.993 0.11-0.973 0.11-0.973 1.606 0.113 2.452 1.649 2.452 1.649 1.427 2.446 3.743 1.739 4.656 1.33 0.143-1.034 0.558-1.74 1.016-2.14-3.554-0.404-7.29-1.777-7.29-7.907 0-1.747 0.625-3.174 1.649-4.295-0.166-0.403-0.714-2.030 0.155-4.234 0 0 1.344-0.43 4.401 1.64 1.276-0.355 2.645-0.532 4.005-0.539 1.359 0.006 2.729 0.184 4.008 0.539 3.054-2.070 4.395-1.64 4.395-1.64 0.871 2.204 0.323 3.831 0.157 4.234 1.026 1.12 1.647 2.548 1.647 4.295 0 6.145-3.743 7.498-7.306 7.895 0.574 0.497 1.085 1.47 1.085 2.963 0 2.141-0.019 3.864-0.019 4.391 0 0.426 0.288 0.925 1.099 0.768 6.354-2.118 10.933-8.113 10.933-15.18 0-8.837-7.164-16-16-16z"
          />
        </symbol>
        <symbol id="icon-wordpress" viewBox="0 0 32 32">
          <title>wordpress</title>
          <path
            d="M4 16c0 4.626 2.76 8.624 6.763 10.518l-5.724-15.274c-0.666 1.453-1.039 3.062-1.039 4.756zM24.101 15.41c0-1.445-0.533-2.445-0.989-3.223-0.608-0.963-1.179-1.778-1.179-2.741 0-1.074 0.836-2.074 2.015-2.074 0.053 0 0.104 0.006 0.155 0.009-2.135-1.905-4.979-3.069-8.103-3.069-4.192 0-7.881 2.095-10.026 5.268 0.282 0.009 0.547 0.014 0.772 0.014 1.255 0 3.198-0.149 3.198-0.149 0.647-0.037 0.723 0.889 0.077 0.963 0 0-0.65 0.074-1.373 0.111l4.37 12.66 2.626-7.671-1.869-4.989c-0.646-0.037-1.259-0.111-1.259-0.111-0.647-0.037-0.571-1 0.076-0.963 0 0 1.981 0.149 3.16 0.149 1.255 0 3.198-0.149 3.198-0.149 0.647-0.037 0.723 0.889 0.076 0.963 0 0-0.651 0.074-1.373 0.111l4.337 12.563 1.197-3.895c0.518-1.617 0.913-2.778 0.913-3.778zM16.211 17.022l-3.601 10.189c1.075 0.308 2.212 0.476 3.39 0.476 1.397 0 2.738-0.235 3.985-0.663-0.032-0.050-0.062-0.103-0.086-0.161l-3.688-9.842zM26.53 10.393c0.052 0.372 0.081 0.771 0.081 1.202 0 1.185-0.228 2.519-0.913 4.186l-3.665 10.321c3.568-2.026 5.967-5.79 5.967-10.102 0-2.032-0.533-3.943-1.47-5.607zM16 0c-8.837 0-16 7.163-16 16s7.163 16 16 16 16-7.163 16-16-7.163-16-16-16zM16 30c-7.732 0-14-6.268-14-14s6.268-14 14-14 14 6.268 14 14-6.268 14-14 14z"
          />
        </symbol>
        <symbol id="icon-linkedin2" viewBox="0 0 32 32">
          <title>linkedin2</title>
          <path
            d="M12 12h5.535v2.837h0.079c0.77-1.381 2.655-2.837 5.464-2.837 5.842 0 6.922 3.637 6.922 8.367v9.633h-5.769v-8.54c0-2.037-0.042-4.657-3.001-4.657-3.005 0-3.463 2.218-3.463 4.509v8.688h-5.767v-18z"
          />
          <path d="M2 12h6v18h-6v-18z" />
          <path
            d="M8 7c0 1.657-1.343 3-3 3s-3-1.343-3-3c0-1.657 1.343-3 3-3s3 1.343 3 3z"
          />
        </symbol>
        <symbol id="icon-logo" viewBox="0 0 32 32">
          <title>logo</title>
          <path d="M0 0h12.8v25.6h-12.8v-25.6z" />
          <path d="M0 24.624h6.4v7.376h-6.4v-7.376z" />
          <path d="M19.2 0h12.8v32h-12.8v-32z" />
          <path d="M12.8 6.4h6.4v1.6h-6.4v-1.6z" />
        </symbol>

        <symbol id="icon-cheese" viewBox="0 0 17 16">
          <!-- Generator: Sketch 3.0.3 (7891) - http://www.bohemiancoding.com/sketch -->
          <title>cheese</title>
          <defs />
          <g stroke="none" stroke-width="1">
            <path
              d="M11.744,2.128 C11.524,2.167 10.306,2.539 8.803,3.213 C9.104,3.677 9.186,4.158 8.961,4.479 C8.615,4.979 7.666,4.895 6.842,4.291 C6.79,4.255 6.746,4.212 6.698,4.173 C3.92,5.454 1.031,6.841 1.031,6.841 L1.031,9.435 C2.238,9.56 3.185,10.558 3.195,11.789 L10.947,12.487 C11.091,11.512 11.677,10.781 12.381,10.781 C13.147,10.781 13.769,11.646 13.838,12.748 L16.268,12.966 L16.51,10.726 C15.783,10.661 15.227,10.329 15.227,9.925 C15.227,9.49 15.874,9.139 16.686,9.115 C16.686,9.115 16.795,8.154 16.934,6.842 C17.238,5.068 13.048,1.899 11.744,2.128 L11.744,2.128 Z M14.534,4.711 C14.997,4.711 15.372,5.065 15.372,5.503 C15.372,5.938 14.997,6.294 14.534,6.294 C14.073,6.294 13.697,5.938 13.697,5.503 C13.697,5.064 14.073,4.711 14.534,4.711 L14.534,4.711 Z M11.531,3.766 C12.097,3.766 12.556,4.078 12.556,4.463 C12.556,4.848 12.097,5.159 11.531,5.159 C10.965,5.159 10.507,4.847 10.507,4.463 C10.507,4.077 10.965,3.766 11.531,3.766 L11.531,3.766 Z M6.517,4.69 C6.975,4.69 7.344,5.038 7.344,5.469 C7.344,5.899 6.975,6.246 6.517,6.246 C6.059,6.246 5.69,5.898 5.69,5.469 C5.689,5.038 6.059,4.69 6.517,4.69 L6.517,4.69 Z M6.506,11.125 C5.619,11.125 4.902,10.623 4.902,10 C4.902,9.379 5.619,8.875 6.506,8.875 C7.391,8.875 8.109,9.379 8.109,10 C8.108,10.623 7.391,11.125 6.506,11.125 L6.506,11.125 Z M16.031,8.031 L13.021,8.031 C12.793,8.513 12.307,9.047 11.515,9.047 C10.721,9.047 10.23,8.514 10.003,8.031 L1.983,8.031 L1.983,6.937 L10.034,6.937 C10.289,6.49 10.712,5.953 11.468,5.953 C12.223,5.953 12.734,6.49 12.989,6.937 L16.031,6.937 L16.031,8.031 L16.031,8.031 Z"
              class="si-glyph-fill"
            />
          </g>
        </symbol>

        <symbol id="icon-glass" viewBox="0 0 32 32">
          <title>glass</title>
          <path
            d="M24.306 0.527c-0.174-0.324-0.513-0.527-0.881-0.527h-14.85c-0.368 0-0.707 0.202-0.881 0.527-1.108 2.063-1.694 4.474-1.694 6.973 0 3.294 1.012 6.401 2.849 8.748 1.42 1.814 3.203 3.017 5.151 3.505v10.247h-3c-0.552 0-1 0.448-1 1s0.448 1 1 1h10c0.552 0 1-0.448 1-1s-0.448-1-1-1h-3v-10.247c1.949-0.488 3.732-1.691 5.151-3.505 1.837-2.347 2.849-5.454 2.849-8.748 0-2.499-0.586-4.91-1.694-6.973zM9.191 2h13.619c0.78 1.653 1.191 3.542 1.191 5.5 0 0.168-0.003 0.334-0.009 0.5h-15.981c-0.006-0.166-0.009-0.332-0.009-0.5-0-1.958 0.41-3.847 1.191-5.5z"
          />
        </symbol>

        <symbol id="icon-mountain" viewBox="0 0 17 17">
          <!-- Generator: Sketch 3.0.3 (7891) - http://www.bohemiancoding.com/sketch -->
          <title>mountain</title>
          <defs />
          <g stroke="none" stroke-width="1">
            <path
              d="M8.012,1.062 L4.035,8.87 L2.709,7.569 C2.709,7.569 -0.305,14 0.063,14 L15.965,14 L15.965,13.998 L12.627,6.898 L11.644,7.51 L8.012,1.062 L8.012,1.062 Z M5.611,7.521 L8.062,2.77 L10.347,6.851 L9.361,7.521 L8.021,6.233 L5.611,7.521 L5.611,7.521 Z"
              class="si-glyph-fill"
            />
          </g>
        </symbol>

        <symbol id="icon-france" viewBox="0 0 32 32">
          <title>france</title>
          <path
            fill="#f0f0f0"
            style="fill: var(--color1, #f0f0f0)"
            d="M32 16c0 8.837-7.163 16-16 16s-16-7.163-16-16c0-8.837 7.163-16 16-16s16 7.163 16 16z"
          />
          <path
            fill="#d80027"
            style="fill: var(--color2, #d80027)"
            d="M32 16c0-6.881-4.344-12.744-10.438-15.006v30.013c6.094-2.262 10.438-8.125 10.438-15.006z"
          />
          <path
            fill="#0052b4"
            style="fill: var(--color3, #0052b4)"
            d="M0 16c0 6.881 4.344 12.744 10.438 15.006v-30.012c-6.094 2.263-10.438 8.125-10.438 15.006z"
          />
        </symbol>
        <symbol id="icon-japan" viewBox="0 0 32 32">
          <title>japan</title>
          <path
            fill="#f0f0f0"
            style="fill: var(--color1, #f0f0f0)"
            d="M32 16c0 8.837-7.163 16-16 16s-16-7.163-16-16c0-8.837 7.163-16 16-16s16 7.163 16 16z"
          />
          <path
            fill="#d80027"
            style="fill: var(--color2, #d80027)"
            d="M22.956 16c0 3.842-3.115 6.956-6.957 6.956s-6.957-3.115-6.957-6.957c0-3.842 3.115-6.957 6.957-6.957s6.957 3.115 6.957 6.957z"
          />
        </symbol>
        <symbol id="icon-switzerland" viewBox="0 0 32 32">
          <title>switzerland</title>
          <path
            fill="#d80027"
            style="fill: var(--color2, #d80027)"
            d="M32 16c0 8.837-7.163 16-16 16s-16-7.163-16-16c0-8.837 7.163-16 16-16s16 7.163 16 16z"
          />
          <path
            fill="#f0f0f0"
            style="fill: var(--color1, #f0f0f0)"
            d="M24.348 13.217h-5.565v-5.565h-5.565v5.565h-5.565v5.565h5.565v5.565h5.565v-5.565h5.565z"
          />
        </symbol>
        <symbol id="icon-taiwan" viewBox="0 0 32 32">
          <title>taiwan</title>
          <path
            fill="#d80027"
            style="fill: var(--color2, #d80027)"
            d="M32 16c0 8.837-7.163 16-16 16s-16-7.163-16-16c5.496-5.496 10.156-10.156 16-16 8.837 0 16 7.163 16 16z"
          />
          <path
            fill="#0052b4"
            style="fill: var(--color3, #0052b4)"
            d="M16 16c0-8.836 0-9.878 0-16-8.836 0-16 7.163-16 16h16z"
          />
          <path
            fill="#f0f0f0"
            style="fill: var(--color1, #f0f0f0)"
            d="M13.913 9.364l-1.954 0.919 1.040 1.892-2.122-0.406-0.269 2.144-1.478-1.576-1.478 1.576-0.269-2.144-2.122 0.406 1.040-1.892-1.954-0.919 1.954-0.919-1.041-1.893 2.122 0.406 0.269-2.143 1.478 1.577 1.478-1.577 0.269 2.143 2.122-0.406-1.041 1.893z"
          />
          <path
            fill="#0052b4"
            style="fill: var(--color3, #0052b4)"
            d="M12.109 9.364c0 1.645-1.333 2.978-2.978 2.978s-2.978-1.333-2.978-2.978c0-1.645 1.333-2.978 2.978-2.978s2.978 1.333 2.978 2.978z"
          />
          <path
            fill="#f0f0f0"
            style="fill: var(--color1, #f0f0f0)"
            d="M9.131 10.951c-0.875 0-1.587-0.712-1.587-1.587s0.712-1.587 1.587-1.587c0.875 0 1.587 0.712 1.587 1.587s-0.712 1.587-1.587 1.587z"
          />
        </symbol>
        <symbol id="icon-france-g" viewBox="0 0 32 32">
          <title>france-g</title>
          <path
            fill="#f0f0f0"
            style="fill: var(--color1, #f0f0f0)"
            d="M32 16c0 8.837-7.163 16-16 16s-16-7.163-16-16c0-8.837 7.163-16 16-16s16 7.163 16 16z"
          />
          <path
            fill="#9e9e9e"
            style="fill: var(--color4, #9e9e9e)"
            d="M32 16c0-6.881-4.344-12.744-10.438-15.006v30.013c6.094-2.262 10.438-8.125 10.438-15.006z"
          />
          <path
            fill="#333"
            style="fill: var(--color5, #333)"
            d="M0 16c0 6.881 4.344 12.744 10.438 15.006v-30.012c-6.094 2.263-10.438 8.125-10.438 15.006z"
          />
        </symbol>
        <symbol id="icon-japan-g" viewBox="0 0 32 32">
          <title>japan-g</title>
          <path
            fill="#f0f0f0"
            style="fill: var(--color1, #f0f0f0)"
            d="M32 16c0 8.837-7.163 16-16 16s-16-7.163-16-16c0-8.837 7.163-16 16-16s16 7.163 16 16z"
          />
          <path
            fill="#333"
            style="fill: var(--color5, #333)"
            d="M22.956 16c0 3.842-3.114 6.956-6.956 6.956s-6.956-3.114-6.956-6.956c0-3.842 3.114-6.956 6.956-6.956s6.956 3.114 6.956 6.956z"
          />
        </symbol>
        <symbol id="icon-switzerland-g" viewBox="0 0 32 32">
          <title>switzerland-g</title>
          <path
            fill="#333"
            style="fill: var(--color5, #333)"
            d="M32 16c0 8.837-7.163 16-16 16s-16-7.163-16-16c0-8.837 7.163-16 16-16s16 7.163 16 16z"
          />
          <path
            fill="#f0f0f0"
            style="fill: var(--color1, #f0f0f0)"
            d="M24.35 13.219h-5.569v-5.569h-5.563v5.569h-5.569v5.563h5.569v5.569h5.563v-5.569h5.569z"
          />
        </symbol>
        <symbol id="icon-taiwan-g" viewBox="0 0 32 32">
          <title>taiwan-g</title>
          <path
            fill="#737373"
            style="fill: var(--color6, #737373)"
            d="M32 16c0 8.837-7.163 16-16 16s-16-7.163-16-16c5.494-5.494 10.156-10.156 16-16 8.837 0 16 7.162 16 16z"
          />
          <path
            fill="#333"
            style="fill: var(--color5, #333)"
            d="M16 16c0-8.838 0-9.881 0-16-8.838 0-16 7.162-16 16h16z"
          />
          <path
            fill="#f0f0f0"
            style="fill: var(--color1, #f0f0f0)"
            d="M13.912 9.363l-1.956 0.919 1.044 1.894-2.125-0.406-0.269 2.144-1.475-1.575-1.481 1.575-0.269-2.144-2.119 0.406 1.037-1.894-1.95-0.919 1.95-0.919-1.037-1.894 2.119 0.406 0.269-2.144 1.481 1.581 1.475-1.581 0.269 2.144 2.125-0.406-1.044 1.894z"
          />
          <path
            fill="#333"
            style="fill: var(--color5, #333)"
            d="M12.113 9.363c0 1.646-1.335 2.981-2.981 2.981s-2.981-1.335-2.981-2.981c0-1.646 1.335-2.981 2.981-2.981s2.981 1.335 2.981 2.981z"
          />
          <path
            fill="#f0f0f0"
            style="fill: var(--color1, #f0f0f0)"
            d="M9.131 10.95c-0.875 0-1.588-0.713-1.588-1.588s0.713-1.588 1.588-1.588c0.875 0 1.588 0.713 1.588 1.588s-0.713 1.588-1.588 1.588z"
          />
        </symbol>

        <symbol id="icon-telegram" viewBox="0 0 32 32">
          <title>telegram</title>
          <path
            d="M16 0c-8.838 0-16 7.162-16 16s7.162 16 16 16 16-7.163 16-16-7.163-16-16-16zM23.863 10.969l-2.625 12.369c-0.181 0.881-0.712 1.087-1.45 0.681l-4-2.956-1.919 1.869c-0.225 0.219-0.4 0.4-0.8 0.4-0.519 0-0.431-0.194-0.606-0.688l-1.363-4.475-3.956-1.231c-0.856-0.262-0.862-0.85 0.194-1.269l15.412-5.95c0.7-0.319 1.381 0.169 1.113 1.25z"
          />
        </symbol>

        <symbol id="icon-stackoverflow" viewBox="0 0 32 32">
          <title>stackoverflow</title>
          <path
            d="M32 20v12h-32v-12h4v8h24v-8zM6 22h20v4h-20zM6.473 17.671l0.866-3.905 19.526 4.328-0.866 3.905zM8.739 9.642l1.69-3.625 18.126 8.452-1.69 3.625zM30.991 11.296l-2.435 3.173-15.867-12.175 1.761-2.294h1.82z"
          />
        </symbol>

        <symbol id="icon-spinner" viewBox="0 0 32 32">
          <title>spinner</title>
          <path
            d="M16 32c-4.274 0-8.292-1.664-11.314-4.686s-4.686-7.040-4.686-11.314c0-3.026 0.849-5.973 2.456-8.522 1.563-2.478 3.771-4.48 6.386-5.791l1.344 2.682c-2.126 1.065-3.922 2.693-5.192 4.708-1.305 2.069-1.994 4.462-1.994 6.922 0 7.168 5.832 13 13 13s13-5.832 13-13c0-2.459-0.69-4.853-1.994-6.922-1.271-2.015-3.066-3.643-5.192-4.708l1.344-2.682c2.615 1.31 4.824 3.313 6.386 5.791 1.607 2.549 2.456 5.495 2.456 8.522 0 4.274-1.664 8.292-4.686 11.314s-7.040 4.686-11.314 4.686z"
          />
        </symbol>

        <symbol id="icon-embed" viewBox="0 0 40 32">
          <title>embed</title>
          <path d="M26 23l3 3 10-10-10-10-3 3 7 7z" />
          <path d="M14 9l-3-3-10 10 10 10 3-3-7-7z" />
          <path
            d="M21.916 4.704l2.171 0.592-6 22.001-2.171-0.592 6-22.001z"
          />
        </symbol>

        <symbol id="icon-magic" viewBox="0 0 32 32">
          <title>magic</title>
          <path
            fill="#aeadad"
            style="fill: var(--color1, #aeadad)"
            d="M17.401 16.604c0.783-0.77 1.621-1.438 1.796-1.269l-5.166-5.166c-0.351-0.351-0.972-0.297-1.391 0.122l-1.702 1.695 0.952-0.952 5.51 5.571z"
          />
          <path
            fill="#949897"
            style="fill: var(--color2, #949897)"
            d="M17.327 16.679v0c0.027-0.027 0.047-0.047 0.074-0.074l-5.51-5.571-0.952 0.952c-0.419 0.419-0.473 1.040-0.122 1.391l5.166 5.166c-0.169-0.176 0.54-1.067 1.344-1.864z"
          />
          <path
            fill="#ae8a50"
            style="fill: var(--color3, #ae8a50)"
            d="M17.401 16.604l10.243 10.365c0.52-0.547 0.999-1.134 1.432-1.756l-9.886-9.879c-0.169-0.169-1.013 0.5-1.789 1.269z"
          />
          <path
            fill="#ae8a50"
            style="fill: var(--color3, #ae8a50)"
            d="M17.327 16.679l10.284 10.325c0.007-0.014 0.020-0.020 0.027-0.034l-10.237-10.365c-0.027 0.027-0.054 0.047-0.074 0.074z"
          />
          <path
            fill="#9b7842"
            style="fill: var(--color4, #9b7842)"
            d="M15.983 18.542v0 0z"
          />
          <path
            fill="#9b7842"
            style="fill: var(--color4, #9b7842)"
            d="M17.327 16.679v0c-0.804 0.804-1.513 1.688-1.344 1.864v0c-0.169-0.176 0.54-1.067 1.344-1.864z"
          />
          <path
            fill="#9b7842"
            style="fill: var(--color4, #9b7842)"
            d="M15.983 18.542v0l9.98 9.98z"
          />
          <path
            fill="#9b7842"
            style="fill: var(--color4, #9b7842)"
            d="M15.983 18.542l9.973 9.98c0.587-0.466 1.134-0.972 1.654-1.519l-10.284-10.325c-0.804 0.797-1.513 1.688-1.344 1.864z"
          />
        </symbol>

        <symbol id="icon-star" viewBox="0 0 32 32">
          <title>star</title>
          <path
            fill="#f1d350"
            style="fill: var(--color5, #f1d350)"
            d="M26.293 31.387c0 0-8.667-8.88-11.44-9.467-2.8-0.56-14.427 4.187-14.427 4.187s8.907-8.853 9.467-11.547c0.533-2.693-4.187-14.32-4.187-14.32s8.533 8.853 11.547 9.467c3.013 0.613 14.32-4.187 14.32-4.187s-8.88 8.533-9.493 11.547c-0.613 2.987 4.213 14.32 4.213 14.32z"
          />
          <path
            fill="#ecc92a"
            style="fill: var(--color6, #ecc92a)"
            d="M0.427 26.107c0 0 8.907-8.853 9.467-11.547 0.533-2.693-4.187-14.32-4.187-14.32s8.533 8.853 11.547 9.467c3.013 0.613 14.32-4.187 14.32-4.187l-31.147 20.587z"
          />
        </symbol>
      </defs>
    </svg>
  </div>
</template>

<script>
export default {
  name: 'SvgSprites',
  data () {
    return {}
  }
}
</script>
